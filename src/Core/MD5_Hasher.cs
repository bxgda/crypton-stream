...
public static class MD5_Hasher
{
    ...
    public static string CalculateMD5(byte[] input)
    {
        ...
        // ispravka da bi se izbegle greske sa velikim fajlovima (ako se koristi int, onda se javlja overflow za preko 512MB))
        ...
        byte[] lengthBytes = BitConverter.GetBytes(bitLength); // bit converter returns little-endian
        Array.Copy(lengthBytes, 0, processedInput, processedInput.Length - 8, 8); // add length in bits
